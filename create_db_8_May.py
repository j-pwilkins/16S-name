...
def main():
    ...
    barcoded_df = create_barcodes(processed_vsearch_df, category_thresholds)
...
##### L1
def create_barcodes(processed_vsearch_df, category_thresholds):
...
def name_each_sequence(input_df, barcode_df, i, category_thresholds):
    ...
    if row == 1:
        ...
        new_line = {'DB#': [row], 'A': [Ao], 'B': [Bo], 'C': [Co], 'D': [Do], 'E': [Eo], 'F': [Fo], 'G': [Go],
                    'H': [Ho],
